...
// 	TscannerToMap.NeedsToAlign
	...
	try 
	{
		...
		Ticp = icp(*newPointCloud, TscannerToMap);//struct ICPSequence: public ICP, Ticp —— scanner to map

		// ISER
// 		{
// 		// extract corrections
// 		PM::TransformationParameters Tdelta = Ticp * TscannerToMap.inverse();//scanner n to scanner n-1
// 		
...
Mapper::DP* Mapper::updateMap(DP* newPointCloud, const PM::TransformationParameters Ticp, bool updateExisting)
{
	...
	if (!updateExisting)//输入的形参
	{
		...
		*newPointCloud = transformation->compute(*newPointCloud, Ticp); //Ticp--scannerTomap
