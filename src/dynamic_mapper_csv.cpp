...
class Mapper
{
	...
	string odomFrame;  //launch中param  odom_frame
	string mapFrame;
...
protected:
	...
	void processCloud_csv(unique_ptr<DP> cloud, const std::string& scannerFrame,int seq);
...
void Mapper::gotScan(const sensor_msgs::LaserScan& scanMsgIn)
{
	if(localizing)
	{
		cout<<"LaserScan range size : "<<scanMsgIn.ranges.size()<<endl;
		const ros::Time endScanTime(scanMsgIn.header.stamp + ros::Duration(scanMsgIn.time_increment * (scanMsgIn.ranges.size() - 1)));
		unique_ptr<DP> cloud(new DP(PointMatcher_ros::rosMsgToPointMatcherCloud<float>(scanMsgIn, &tfListener, odomFrame)));//得到的点云是在odomFrame坐标系下的，筛选了ranges的大小是否符合条件
		processCloud(move(cloud), scanMsgIn.header.frame_id, endScanTime, scanMsgIn.header.seq);//scanMsgIn.header.frame_id是/laser
	}
}
...
void Mapper::readCloud(int num)
{
// 	cout<<"Processing "<<num<<" and "<<(num+1)<<" CSV"<<endl;
	cout<<"Processing "<<num<<" CSV"<<endl;
	stringstream s;
	s<<"/home/dxq/catkin_ws/src/ethzasl_icp_mapping/ethzasl_icp_mapper/bag/local_frame/";
	s<<"Hokuyo_"<<num<<".csv";
	unique_ptr<DP> cloud(new DP(PM::DataPoints::load(s.str())));
	processCloud_csv(move(cloud), "/laser",num);
...
void Mapper::processCloud(unique_ptr<DP> newPointCloud, const std::string& scannerFrame, const ros::Time& stamp, uint32_t seq)
{
	...
	BoolSetter stopProcessingSetter(processingNewCloud, false);//(target, toSetValue)
	...
	ROS_INFO("Processing new point cloud");
	{
		...
		cout<<"Before inputFilter are "<<newPointCloud->features.cols()<<" points"<<endl;
		inputFilters.apply(*newPointCloud);
		cout<<"After inputFilter are "<<newPointCloud->features.cols()<<" points"<<endl;
...
// 	ROS_DEBUG_STREAM("TOdomToMap(" << odomFrame<< " to " << mapFrame << "):\n" << TOdomToMap);
	cout<<"TOdomToScanner(" << odomFrame<< " to " << scannerFrame << "):\n" << TOdomToScanner<<endl;
	cout<<"TOdomToMap(" << odomFrame<< " to " << mapFrame << "):\n" << TOdomToMap<<endl;
...
// 	ROS_DEBUG_STREAM("TscannerToMap (" << scannerFrame << " to " << mapFrame << "):\n" << TscannerToMap);
	cout<<"TscannerToMap (" << scannerFrame << " to " << mapFrame << "):\n" << TscannerToMap<<endl;
// 	TscannerToMap.NeedsToAlign
	...
	try 
	{
		...
		Ticp = icp(*newPointCloud, TscannerToMap);//struct ICPSequence: public ICP
...
void Mapper::processCloud_csv(unique_ptr< Mapper::DP > newPointCloud, const string& scannerFrame, int seq)
{
	processingNewCloud = true;
	BoolSetter stopProcessingSetter(processingNewCloud, false);//(target, toSetValue)
...
// 	cerr << "received new map" << endl;
	...
	cout<<"newPointCloud cols one "<<newPointCloud->features.cols()<<endl;
	...
	// Initialize the transformation to identity if empty
 	...
 	{
		// we need to know the dimensionality of the point cloud to initialize properly
		cout<<"Initialize TOdomToMap "<<endl;
		publishLock.lock();//boost::mutex publishLock;
		TOdomToMap = PM::TransformationParameters::Identity(dimp1, dimp1);
		TOdomToMap(2,3) = mapElevation;
		publishLock.unlock();
